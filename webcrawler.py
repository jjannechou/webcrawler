...
def main():
    for year in ['2010s', '2000s', '1990s']:
        ...
        for tag in tags:
            total = tag.tbody.text.split()
            count = 0
            for ele in total:
                count += 1
                if count % 5 == 3:    # male
                    male_number = int(ele.replace(',', ''))
                    total_male += male_number
                elif count % 5 == 0:    # female
                    female_number = int(ele.replace(',', ''))
                    total_female += female_number
                if count > 1000:
                    break
